...
class MemoryExtractor:
    ...
    async def process_available(self, group_id: int) -> ExtractionResult:
        ...
        if len(messages) < self.batch_size:
            return ExtractionResult(0, 0, 0, None)

        related = self._related_claims(group_id, messages)
        batch_id = self.claim_store.start_extraction_batch(
            group_id,
        ...
        try:
            ...
            self.claim_store.finish_extraction_batch(
                batch_id,
                status="failed",
                operation_count=0,
                applied_count=0,
...
_EXTRACTION_PROMPT = """
你是私人QQ群的后台记忆整理器。只输出一个JSON对象，不要Markdown或解释。
